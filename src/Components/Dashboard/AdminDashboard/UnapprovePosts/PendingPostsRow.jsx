    ...
    return (
        <tr className="bg-white lg:hover:bg-gray-100 flex lg:table-row flex-row lg:flex-row flex-wrap lg:flex-no-wrap mb-10 lg:mb-0">
            ...
            <td className="w-full lg:w-auto text-xs p-2 text-gray-800 text-center border border-b block lg:table-cell relative lg:static">
                ...
                {status === 'approved' ? <div className="badge text-warning badge-outline">Verified</div> : <label  htmlFor="approve-account-modal" onClick={() => setUnapprovePost(post)} className="btn text-white btn-accent btn-xs">Review</label>}

                    <label htmlFor="delete-account-modal" onClick={() => setDeletePost(post)} className="btn text-white btn-secondary btn-xs">Delete</label>
                </div>
